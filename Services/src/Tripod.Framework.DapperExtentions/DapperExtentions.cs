...
using Dapper;
using Tripod.Framework.DapperExtentions.Adapters;
using Tripod.Framework.DapperExtentions.Attributes;

namespace Tripod.Framework.DapperExtentions
{
    /// <summary>
    ...
    public static partial class SqlMapperExtensions
    {
        ...
        private static readonly ConcurrentDictionary<string, string> PropertyColumnName = new ConcurrentDictionary<string, string>();
        ...
        // NOTE 目前的优先级存在问题，应该是范围越大的优先级越低，而现在当同时指定了列名映射委托和特性时，特性是不生效的，需要调整！(TableName的获取同样存在问题)
        private static string GetColumnName(PropertyInfo propertyInfo)
        {
            string cacheKey = $"{propertyInfo.DeclaringType.FullName}.{propertyInfo.Name}";
            if (PropertyColumnName.TryGetValue(cacheKey, out string name)) return name;
            ...
            PropertyColumnName[cacheKey] = name;
            return name;
        }
