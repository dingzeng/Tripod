...
CREATE TABLE `supplier` (
    ...
    `settle_date` VARCHAR(45) NULL COMMENT '结算日期',
    `status` INT NOT NULL DEFAULT(0) COMMENT '状态（0-正常、9-停用）',
    `contacts_name` VARCHAR(45) NULL COMMENT '联系人',
    `contacts_mobile` VARCHAR(45) NULL COMMENT '移动电话',
    `contacts_tel` VARCHAR(45) NULL COMMENT '固定电话',
    `contacts_email` VARCHAR(45) NULL COMMENT 'Email',
    `account_bank` VARCHAR(45) NULL COMMENT '开户行',
    `account_no` VARCHAR(45) NULL COMMENT '账号',
    `tax_registration_no` VARCHAR(45) NULL COMMENT '税务登记号',
    `business_license_no` VARCHAR(45) NULL COMMENT '营业执照号',
